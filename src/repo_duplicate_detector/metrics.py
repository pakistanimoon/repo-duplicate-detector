...
class SimilarityMetrics:
    ...
    def __init__(self, use_semantic: bool = True, model_name: str = "all-MiniLM-L6-v2"):
        ...
        if self.use_semantic:
            try:
                ...
                logger.warning(f"Failed to load semantic model: {e}. Falling back to text matching.")
    ...
    def calculate_contributor_overlap(self, contributors1: List[str], contributors2: List[str]) -> float:
        """
    ...
    def calculate_overall_similarity(
        ...
        desc_sim = self.calculate_description_similarity(
            repo1.get("description", ""),
            repo2.get("description", "")
        )
        ...
        contrib_sim = self.calculate_contributor_overlap(
            repo1.get("contributors", []),
            repo2.get("contributors", [])
        )
